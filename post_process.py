...
class ProcessBase(IProcess):
    """
    Base process class implementing the common parts of IProcess. Concrete processes subclass
    this and only need to implement execute().

    *Args:
        config (ProcessBaseConfig)  :   Supplies result_name (key this process's result is stored
                                        under in context.result_repository.data) and data_slice
                                        (which rows of context.raw_data this process operates on).
    """
...
class ProcessAbsoluteMax(ProcessBase):
    """
    Computes the absolute maximum (peak magnitude, ignoring sign and NaNs) of each signal in its data slice.
    """
    def execute(self, context: ProcessContext):
        """
        *Args:
            context (ProcessContext)    :   Supplies raw_data to read from and result_repository to write the absolute max into.
        """
...
class ProcessAverage(ProcessBase):
    """
    Computes the arithmetic mean (ignoring NaNs) of each signal in its data slice.
    """
    def execute(self, context: ProcessContext):
        """
        *Args:
            context (ProcessContext)    :   Supplies raw_data to read from and result_repository to write the mean into.
        """
        data                                                                    = context.raw_data[self.data_slice]
...
class ProcessPeak(ProcessBase):
    """
    Computes the maximum value (ignoring NaNs) of each signal in its data slice.
    """
    def execute(self, context: ProcessContext):
        """
        *Args:
            context (ProcessContext)    :   Supplies raw_data to read from and result_repository to write the peak into.
        """
        data                                                                    = context.raw_data[self.data_slice]
...
class ProcessRMS(ProcessBase):
    ...
    def execute(self, context: ProcessContext):
        """
        *Args:
            context (ProcessContext)    :   raw_data[0] is the time vector; raw_data[self.data_slice] are the signals to RMS.
        """
...
class ProcessAVG(ProcessBase):
    ...
    def execute(self, context: ProcessContext):
        """
        *Args:
            context (ProcessContext)    :   raw_data[0] is the time vector; raw_data[self.data_slice] are the signals to average.
        """
        #* Calculate the time interval (delta_T) based on the first and last time values.
        t_vec   = context.raw_data[0]
        data    = context.raw_data[self.data_slice]
...
class ProcessNegativePeak(ProcessBase):
    """
    Computes the minimum value (ignoring NaNs) of each signal in its data slice.
    """
    def execute(self, context: ProcessContext):
        """
        *Args:
            context (ProcessContext)    :   Supplies raw_data to read from and result_repository to write the minimum into.
        """
        data                                                                    = context.raw_data[self.data_slice]
...
class ProcessMax(ProcessBase):
    """
    Computes the maximum value of each signal in its data slice (does not ignore NaNs -- unlike NMAX/ProcessPeak).
    """
    def execute(self, context: ProcessContext):
        """
        *Args:
            context (ProcessContext)    :   Supplies raw_data to read from and result_repository to write the maximum into.
        """
        data                                                                    = context.raw_data[self.data_slice]
...
class ProcessMin(ProcessBase):
    """
    Computes the minimum value of each signal in its data slice (does not ignore NaNs -- unlike NMIN/ProcessNegativePeak).
    """
    def execute(self, context: ProcessContext):
        """
        *Args:
...
class ProcessPtP(ProcessBase):
    """
    Computes the peak-to-peak range (max minus min) of each signal in its data slice.
    """
    def execute(self, context: ProcessContext):
        """
        *Args:
            context (ProcessContext)    :   Supplies raw_data to read from and result_repository to write the peak-to-peak range into.
        """
        data = context.raw_data[self.data_slice]
...
class ProcessFirst(ProcessBase):
    """
    Takes the first sample of each signal in its data slice.
    """
    def execute(self, context: ProcessContext):
        """
        *Args:
            context (ProcessContext)    :   Supplies raw_data to read from and result_repository to write the first sample into.
        """
        data = context.raw_data[self.data_slice]
...
class ProcessLast(ProcessBase):
    """
    Takes the last sample of each signal in its data slice.
    """
    def execute(self, context: ProcessContext):
        """
        *Args:
            context (ProcessContext)    :   Supplies raw_data to read from and result_repository to write the last sample into.
        """
        data = context.raw_data[self.data_slice]
...
class ProcessFFT(ProcessBase):
    ...
    *Args:
        config (ProcessFFTConfig)   :   Supplies hrmcs (which harmonic orders to keep), to_file_ts (resample step; 0 means resample),
                                        and fund_freq (fundamental frequency in Hz, used to locate harmonics in the spectrum).
    """
    ...
    def execute(self, context: ProcessContext):
        """
        *Args:
            context (ProcessContext)    :   raw_data[0] is the time vector; raw_data[self.data_slice] are the signals to FFT.
        """
        t_vec               = context.raw_data[0]
        data                = context.raw_data[self.data_slice]
...
class ProcessDissipation(ProcessBase):
    ...
    *Args:
        config (ProcessDissipationsConfig) :   Supplies current_idx (how many of the RMS current results to
                                                include in res_dissip). Also carries res_list, though execute()
                                                actually reads the resistance values from context.res_list instead.
    """
    ...
    def execute(self, context: ProcessContext):
        """
        *Args:
            context (ProcessContext)    :   Reads "AVG_Dissipations" and "RMS_Currents" from result_repository;
                                            writes the combined total dissipation into result_repository under self.result_name.
        """
...
class ProcessThermalStats(ProcessBase):
    ...
    *Args:
        config (ProcessThermalStatsConfig) :   Supplies the rail/common/output-power result indices, auxiliary power draw,
                                                and phase count needed to compute efficiency and total losses.
    """
    ...
    def execute(self, context: ProcessContext):
        """
        *Args:
            context (ProcessContext)    :   Reads "Dissipations" and "Elec_Stats" from result_repository; writes total dissipation,
                                            efficiency (with and without auxiliary power), and input power into result_repository under self.result_name.
                                            Writes an empty (1,) result instead if Thermal_Stats results are unavailable or this is a dual-rail (dcdc_d) model.
        """
...
class ProcessSensitivity(ProcessBase):
    """
    Calculate Sensitivity on parent map: the percent change in a parent process's result relative
    to its own baseline (thread_index 0), for each perturbed parameter.

    *Args:
        config (ProcessSensitivityConfig)  :   Supplies perturbation (the parameter perturbation size, as a percent), fft_cfg
                                                (used to detect whether the parent result is FFT-shaped), and post_fix (the
                                                suffix stripped from this process's own name to find the parent process's result).
    """
    ...
    def execute(self, context: ProcessContext):
        """
        *Args:
            context (ProcessContext)    :   thread_index 0 captures the parent process's baseline result (Y0); each later thread_index
                                            reads the parent's perturbed result and writes the percent sensitivity into result_repository
                                            under self.result_name.
        """
