...
def simScript(OptStruct,Thread,Map,iterNumber,ResultsPath,misc,crash=False):
    ...
    *Args:
    ...
    #?  to assign a sweep parameter to a simulation parameter use the following command:
        #*  mdlVars['DesiredParameter']                     =   mapVars[X1][x]  for WCA case
        #*  mdlVars['DesiredParameter']                     =   mapVars[X1]     for Normal sweep case

            #   x is the parameter index as created in the 'Input_vars.json' file, ranging from 0 to n
            #   where n is the number of values in a given sublist of values in the corresponding X list.
            #   X1...X10 is the list of values defined in the input_vars file.
