...
PSFBconfigs			= 	{																																	#*	PSFB components model configuration
                        ...
                        'Io_MeasLoc'				: 0																									,	#?	LV Io measurement location, 0->after caps | 1->before caps
                        }
...
Control				=	{																																	#*	reference and control targets
						...
						'Targets'	:{																														#!	control loop target values
							...
							'Pcmax'					: 2295																								,	#?	Maximum saturated buck output power
							'Icmax'       			: 170      																								#?	Maximum saturated buck inductor current
									},
...
Trafo				=	copy.deepcopy(Mags['Cyntec_EB2_Trafo'])																							#*	main transformer parameters
...
RCsnubber			=	{																																	#*	RC snubbers parameters
						...
						'LV_FB'		:	{																													#!	LV full-bridge RC snubbers parameters
							...
							'Cs'		:	{																												#	snubber capacitors
								'Config'    		:	2																								,   #?	1->capacitor only | 2->include ESR | 3->include ESR & ESL | 4->variable capacitor | 5->pass | 6->short
								'Csingle'   		:	2.2e-9																							,   #?	single capacitor value
								'Rsingle'   		:	10e-3																							,   #?	ESR of single capacitor
...
RCDclamp			= 	{																																	#*	RCD clamp parameters
                        ...
                        'Switch'     				:	copy.deepcopy(Switches['SQJQ186ER_2'])														,	#?	clamp switch
					}
...
FRW					=	{																																	#*	freewheeler parameters
						'Config'					:	2																								,	#?	1->enable | 2->disable
						'Switch'					: 	copy.deepcopy(Switches['SQJQ186ER_2'])														,	#?	freewheeling switch parameters
...
Lf					= 	copy.deepcopy(Mags['Cyntec_EB2_Choke'])																							#*	LC filter output choke parameters
...
SC_Protection 		=	{																																	#*	internal short-circuit protection parameters
							'Switch'						:	copy.deepcopy(Switches['SQJQ144AER'])													,	#?	short-circuit switch parameters
...
LVcmc				= 	{																																	#*	LV CMC choke parameters
							'Config'				:	3																								,	#?	1->positively coupled | 2->negatively coupled | 3->pass
							'Lself' 				:	0																								,   #?	windings self inductance
							'Cwind'					:	0.0																								,	#?	intrawinding capacitance
							'Rwind' 				:	0.0			  																					,   #?	windings resistance
...
Coc2				=	{																																	#*	LV filter second ceramic capacitors parameters
							...
							'Rsingle'  				:	4e-3																							,   #?	ESR of single capacitor
							'Lsingle'  				:	75e-9																							,   #?	ESL of single capacitor
...
FaultLogic			=	{																																	#* hardware fault logic (safe-state aggregator) parameters
                            ...
                            'TrafoCurrent'	:	{																											#!	trafo current fault logic parameters
								...
								'UpBand'				:	101																							,	#?	upper heysteresis band
								'LoBand'				:	100																							,	#?	lower heysteresis band
								'Delay'					:	614e-9																						,	#? 	overall propagation delay
								'Vcc'					:	3.3																							,	#?	upper rail supply voltage
								'Vee'					:	0.04																						,	#?	lower rail supply voltage
								'Vin'					:	3.0																								#?	input reference voltage
...
LV_currentSense 	= 	copy.deepcopy(Sensors['INA240A1'])																								#*	LV current sensor
...
Rectifier_1			=	copy.deepcopy(Switches['SQJQ186ER_1'])																							#*	LV rectifier switches parameters
Rectifier_2			=	copy.deepcopy(Switches['SQJQ186ER_1'])																							#*	LV rectifier switches parameters
Rectifier_3			=	copy.deepcopy(Switches['SQJQ186ER_1'])																							#*	LV rectifier switches parameters
Rectifier_4			=	copy.deepcopy(Switches['SQJQ186ER_1'])																							#*	LV rectifier switches parameters
...
Thermal 			=   {																																	#* 	thermal conditions
                            ...
                            'Derating'	:	{																												#*	boundary coniditions for derating functions
                                ...
                                'Twater_nom'		:	35																								,	#?	nom water temperature
                                'Twater_max'		:	75																								,	#?	max water temperature
                                'Pmax_2s_nom'		:	2295																							,	#?	max 2-sec power at 35°C water
                                'Pmax_2s_max'		:	1331																							,	#?	max 2-sec power at 75°C water
                                'Pmax_10s_nom'		:	1620																							,	#?	max 10-sec power at 35°C water
                                'Pmax_10s_max'		:	1330																							,	#?	max 10-sec power at 75°C water
                                'Pcont_nom'			:	1276																							,	#?	max continuous power at 35°C water
                                'Pcont_max'			:	1134																								#?	max continuous power at 75°C water
							},
