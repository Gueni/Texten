...
Battery		= 	copy.deepcopy(Battery_Dicts.AllBatteries)
Diode		= 	copy.deepcopy(Diodes_Dicts.AllDiodes)
Beads 		= 	copy.deepcopy(FerriteBeads_Dicts.AllFerriteBeads)
...
Probes				= 	{																																	#*	probes statuses
                            ...
                            'Control_Probes'      	: 5       		  																						#? 	1->Peak Buck | 2->Duty Precharge | 3->Active Clamp Precharge | 4->Duty Boost | 5->Duty Buck | 6->Disable
                        }
...
ToFile				=	collections.OrderedDict({																													#*	PLECS built-in data output configuration
							...
							'Y11'					:	{																									#!	data set 11
                  			...
                  			'Y13'					:	{																									#!	data set 11
								'Config'						: 2																						,	#?	1->Enable | 2->Disable
								'Length'						: 167																						#?	length of export data
							}
           				})
...
PSFBconfigs			= 	{																																	#*	PSFB components model configuration
                        ...
                        'ControlConfig'    	  		: 1				  																					,   #? 	1->Peak Buck | 2->Duty Precharge | 3->Active Clamp Precharge | 4->Duty Boost | 5->Duty Buck | 6->Open-Loop Buck | 7->Average Peak Buck
                        ...
                        'ShortCircuit'    	    	: 3       																							,	#?	1->MOSFET | 2->Resistive | 3->Pass
                        ...
                        'Io_MeasLoc'				: 1																									,	#?	LV Io measurement location, 0->after caps | 1->before caps
                        }
...
Control				=	{																																	#*	reference and control targets
						...
						'Targets'	:{																														#!	control loop target values
							...
							'Pcmax'					: 3500																								,	#?	Maximum saturated buck output power
							'Icmax'       			: 350      																								#?	Maximum saturated buck inductor current
									},
						'Inputs'   :{																														#!	input referece values
							'Vin'        			: 425																								,	#?	constant rail input voltage
...
RailsEnable 		=	{																																	#*	rails enabling configurations
							'Rail_1'  :	{																													#!	rail 1 (lower rail) configuration
								'TimeVec_Buck'					:	[0, 1]																				,	#?	buck mode activation time vector
								'OutVec_Buck'					:	[1, 1]																				,	#? 	buck mode status 0->disabled | 1->enabled
...
HVdmc				= 	copy.deepcopy(Mags['Fastron_1R8'])																								#*	HVdmc filter choke parameters
...
Cpi 				=	{																																	#*	HV Pi capacitor parameters
							'Config'    			:	2																								, 	#?	1->capacitor only | 2->include ESR | 3->include ESR & ESL | 4->variable capacitor | 5->pass | 6->short
							'Csingle'   			:	3.3e-6																							,	#?	single capacitor value
							'Rsingle'   			:	17e-3																							,	#?	ESR of single capacitor
							'Lsingle'   			:	17.0e-9																							,	#?	ESL of single capacitor
							'nPar' 	   				:	1						 																		,  	#?	number of parallel connections
							'nSer' 	  		 		:	1																								, 	#?	number of series connections
							'Vinit'     			:	Initials['Vin']        																			,	#?	capacitor initial voltage
                            'Rd'					:	"inf"																							,	#?	parallel discharge resistor
...
Cyi					= 	{																																	#*	HV Y-capacitors parameters
							...
							'Rsingle'   			:	2.2  																							,	#?	ESR of single capacitor
...
Cc					= 	{																																	#*	HV snubber capacitors parameters
							'Config'    			:	2																								,   #?	1->capacitor only | 2->include ESR | 3->include ESR & ESL | 4->variable capacitor | 5->pass | 6->short
							'Csingle'   			:	5.6e-6																							,   #?	single capacitor value
							'Rsingle'   			:	10.7e-3																							,   #?	ESR of single capacitor
							'Lsingle'   			:	22.0e-9																							,   #?	ESL of single capacitor
							'nPar' 	   				:	1																								,   #?	number of parallel connections
							'nSer' 	  		 		:	1																								,   #?	number of series connections
							'Vinit'     			:	Initials['Vin']		       																			#?	capacitor initial voltage
						}

Trafo				=	copy.deepcopy(Mags['Gen6WE_A10_Trafo'])																							#*	main transformer parameters
...
Cb					= 	{																																	#*	transformer blocking capacitor parameters
							...
							'nSer' 	    			:	1																								,   #?	number of series connections
							'Vinit'     			:	0				                																,   #?	capacitor initial voltage
							'Rd'     				:	10e3				                															,   #?	parallel discharge resistance
						}
...
RCsnubber			=	{																																	#*	RC snubbers parameters
						'SR_MOSFET'	:	{																													#!	SR MOSFETs RC snubbers parameters
							...
							'Cs'		:	{																												#	snubber capacitors
								...
								'Csingle'   		:	4.7e-9																							,   #?	single capacitor value
								'Rsingle'   		:	10e-3																							,   #?	ESR of single capacitor
								'Lsingle'   		:	51.15e-12																						,   #?	ESL of single capacitor
								'nPar' 	    		:	1																								,   #?	number of parallel connections
								'nSer' 	    		:	1																								,   #?	number of series connections
								'Vinit'     		:	0																									#?	capacitor initial voltage
							}
                        ...
                        'PS_MOSFET'	:	{																													#!	PS MOSFETs RC snubbers parameters
							...
							'Cs'		:	{																												#	snubber capacitors
								'Config'    		:	1																								,   #?	1->capacitor only | 2->include ESR | 3->include ESR & ESL | 4->variable capacitor | 5->pass | 6->short
								'Csingle'   		:	1e-9																							,   #?	single capacitor value
								'Rsingle'   		:	0																								,   #?	ESR of single capacitor
								'Lsingle'   		:	0																								,   #?	ESL of single capacitor
								'nPar' 	    		:	1																								,   #?	number of parallel connections
								'nSer' 	    		:	1																								,   #?	number of series connections
						...
						'HV_FB'		:	{																													#!	HV full-bridge RC snubbers parameters
							...
							'Cs'		:	{																												#	snubber capacitors
								...
								'Rsingle'   		:	1.5																								,   #?	ESR of single capacitor
								'Lsingle'   		:	450e-12																							,   #?	ESL of single capacitor
								'nPar' 	    		:	1																								,   #?	number of parallel connections
								'nSer' 	    		:	1																								,   #?	number of series connections
								'Vinit'     		:	0																									#?	capacitor initial voltage
							}
                        ...
                        'HV_Filter'	:	{																													#!	HV filter RC snubbers parameters
...
RCDclamp			= 	{																																	#*	RCD clamp parameters
						'Config'					:	1																								,	#!	1->Enable | 2->Disable
                        'Rs'		:	{																											        #!	snubber resistors
							'R'						:	2.5e3																							,	#?	resistance value
						},
						'Cs'		:	{																										            #!	snubber capacitors
							'Config'    			:	2																								,   #?	1->capacitor only | 2->include ESR | 3->include ESR & ESL | 4->variable capacitor | 5->pass | 6->short
							'Csingle'   			:	2.34e-6																							,   #?	single capacitor value
							'Rsingle'   			:	8.888e-3																						,   #?	ESR of single capacitor
							'Lsingle'   			:	1e-9																							,   #?	ESL of single capacitor
							'nPar' 	    			:	10																								,   #?	number of parallel connections
							'nSer' 	    			:	1																								,   #?	number of series connections
							'Vinit'     			:	0																									#?	capacitor initial voltage
						},
                        'Switch'     				:	copy.deepcopy(Switches['NVMJST3D3N08X'])														,	#?	clamp switch
					}
...
Lf					= 	copy.deepcopy(Mags['Gen6WE_A10_Choke'])																							#*	LC filter output choke parameters

Co					=	{																																	#*	LC filter cermaic capacitors parameters
							'Config'   				:	2				   																				,   #?	1->capacitor only | 2->include ESR | 3->include ESR & ESL | 4->variable capacitor | 5->pass | 6->short
							'Csingle'  				:	13.67e-6   																						,   #?	single capacitor value
							'Rsingle'  				:	3.244e-3																						,   #?	ESR of single capacitor
							'Lsingle'  				:	1.15e-9			   																				,   #?	ESL of single capacitor
							'nPar' 	   				:	13																								,   #?	number of parallel connections
...
SC_Protection 		=	{																																	#*	internal short-circuit protection parameters
							'Switch'						:	copy.deepcopy(Switches['NVMJST0D5N04XM'])											,	#?	short-circuit switch parameters
...
SC_Protection['Switch']['Transistor']['RdsonScale'] = 0
SC_Protection['Switch']['Transistor']['IdsonScale'] = 0
SC_Protection['Switch']['BodyDiode']['VIcurve']['Vfscale'] = 0
SC_Protection['Switch']['Rth_jc'] = 0
SC_Protection['Switch']['Cth_jc'] = 0
SC_Protection['Switch']['Rth_ca'] = 0
SC_Protection['Switch']['Cth_ca'] = 0
...
LVdmc				= 	{																																	#*	LV DMC choke parameters
							'Config'				:	1																								,	#?	1->positively coupled | 2->negatively coupled | 3->pass
							'Lself' 				:	100e-9																							,   #?	windings self inductance
							'Cwind'					:	0.0																								,	#?	intrawinding capacitance
							'Rwind'					:	0.0																								,   #?	windings resistance
							'Km'    				:	0.0																								,  	#?	coupling factor
							'Cm'					:	0.0																								,	#?	interwinding capacitance
							'Rm'					:	0			 		            																,   #?	mutual resistance
							'N'						:	1																									#?	number of turns
						}
...
Coc3				=	{																																	#*	LV filter third ceramic capacitors parameters
							...
							'Rsingle'  				:	3.244e-3																						,   #?	ESR of single capacitor
							'Lsingle'  				:	1.15e-9																							,   #?	ESL of single capacitor
							'nPar' 	   				:	11																								,   #?	number of parallel connections
							'nSer' 	   				:	2																								,   #?	number of series connections
							'Vinit'    				:	Initials['Vout']	            																    #?	capacitor initial voltage
						}
...
Busbars_PCB			=	{																																	#*	PCB traces and/or busbars resistances
							'LV_Filter'		:	{																											#!	LV filter inline resistances
                                'PlusResistance'		:	0																							,	#?	positive line resistance
...
LVS					=	{																																	#*	LV battery parameters
                            ...
                            'Cell'	:	{																													#!	battery cell parameters
								'Model'				:	copy.deepcopy(Battery['CAMEL_230906_warm_BOL'])												,	#?	cell electrical parameters
...
PWM					= 	{																																	#*	microcontroller PWM parameters
							...
							'RecScheme'      		: 	2                               																,   #?	SR modulation, 0->passive modulation | 1->disable CCM modulation | 2->enable CCM modulation | 3->disable DCM modulation
                            'ActScheme'      		: 	0                               																,   #?	PS modulation, 0->disable DCM modulation | 1->enable DCM modulation | 2->disable CCM modulation
...
activeDischarge		= 	{																																	#*	active discharge control
							'Config'					:	3																							,	#?	1->Complex | 2->Simple | 3->Disabled
...
FaultLogic			=	{																																	#* hardware fault logic (safe-state aggregator) parameters
        					...
        					'OutputVoltage'	:	{																											#!	output voltage fault logic parameters
								'R1'					:	1.575949367e3																				,	#?	reference voltage divider supply resistance
								'Rfil'					:	1e3																							,	#?	input signal filter resistance
								'Cfil'					:	10e-9																						,	#?	input signal filter capacitance
								'UpBand'				:	21.87																						,	#?	upper heysteresis band
								'LoBand'				:	19.08																						,	#?	lower heysteresis band
							...
							'OutputCurrentPos':	{																											#!	output voltage fault logic parameters
								'R1'					:	1e3																							,	#?	reference voltage divider supply resistance
								'Rfil'					:	0																							,	#?	input signal filter resistance
								'Cfil'					:	0																							,	#?	input signal filter capacitance
								'UpBand'				:	500																							,	#?	upper heysteresis band for positive currents
								'LoBand'				:	499																							,	#?	lower heysteresis band for positive currents
							...
							'OutputCurrentNeg':	{																											#!	output voltage fault logic parameters
								...
								'Rfil'					:	0																							,	#?	input signal filter resistance
								'Cfil'					:	0																							,	#?	input signal filter capacitance
								'UpBand'				:	-500																						,	#?	upper heysteresis band for negative currents
								'LoBand'				:	-501																						,	#?	lower heysteresis band for negative currents
								'Delay'					:	52e-9																						,	#? 	overall propagation delay
								'Vcc'					:	3.3																							,	#?	upper rail supply voltage
								'Vee'					:	0.04																						,	#?	lower rail supply voltage
								'Vin'					:	3.0																								#?	input reference voltage
							...
							'InputVoltage'	:	{																											#!	input voltage fault logic parameters
								'R1'					:	2.186256781e3																				,	#?	reference voltage divider supply resistance
                            ...
                            'TrafoCurrent'	:	{																											#!	trafo current fault logic parameters
								'R1'					:	1.952380952e3																				,	#?	reference voltage divider supply resistance
								'Rfil'					:	10e3																						,	#?	input signal filter resistance
								'Cfil'					:	100e-12																						,	#?	input signal filter capacitance
								'UpBand'				:	27																							,	#?	upper heysteresis band
								'LoBand'				:	25																							,	#?	lower heysteresis band
								'Delay'					:	614e-9																						,	#? 	overall propagation delay
								'Vcc'					:	3.3																							,	#?	upper rail supply voltage
								'Vee'					:	0.04																						,	#?	lower rail supply voltage
								'Vin'					:	3.0																								#?	input reference voltage
...
ADCmodel			=	{
                            ...
                            'Avg'					:	2																								,	#?	1-> enable average | 2->disable average
                            ...
                            'InputFilters'	:	{																											#*	ADC input filters parameters
								'LV_voltageSense'	:	{																									#!	LV voltage sensor filter parameters
                                    ...
                                    'Cfil'			:	1e-9																								#?	filter capacitance
								},
                                'AC_voltageSense'	:	{																									#!	AC caü voltage sensor filter parameters
                                ...
                                'LV_currentSense'	:	{																									#!	LV current sensor filter parameters
                                    ...
                                    'Cfil'			:	33e-9																								#?	filter capacitance
								},
                                'HV_currentSense'	:	{																									#!	HV current sensor filter parameters
									'Rfil'			:	0																								,	#?	filter resistance
                                    'Cfil'			:	0																									#?	filter capacitance
								},
                                'CT'				:	{																									#!	CT current sensor filter parameters
									'Rfil'			:	200																								,	#?	filter resistance
                                    'Cfil'			:	390e-12																								#?	filter capacitance
								}
							}
						}
...
PCMCbuck 			=	{																																	#*	peak buck mode controller
                            ...
                            'w_p0'					:	2*np.pi*2.5e3																				,	#?	gain of origin pole for Type II
                            'f_p1'					:	1.5e3																							,	#?	pole frequency for Type II
                            'f_z1'					:	40e3																							,	#?	zero frequency for Type II
                            'Td'					:	0																								,	#?	controller delay
							'UpLim'			 		:	2*2**ADCres['Ndac'] - 1																			,	#?	upper saturation limit
							'LoLim'			 		:	0.0	     		  																				,	#?	lower saturation limit
							'CCM_Hi'	  			:	18																								,	#?	current threshold to switch to CCM mode
							'CCM_Lo'	  			:	12																								,	#?	current threshold to switch to DCM mode
...
ACMCprecharge		= 	{																																	#*	precharge mode controller
                            ...
                            'tonsnub_final'			:   250e-9																							,	#?	applied on-time for the active snubber
                            'Points'				:   20																								,	#?	number of points for the current-voltage precharge lookup
							'ohmicOffset'			:	0																								,	#?	add ohmich offset to the reference voltage
							'enableOffset'			:	0																									#?	0->disable | 1->enable
						}

#* Digital AC precharge controller
ACprecharge		= 	{																																		#*	precharge mode controller
							'Config'				:	1																								,	#?	1->discrete-time domain | 2->continuous-time domain
							...
							'Vdelta'         		:	5                	            																,   #?	undervoltage difference where slope starts
                            'Vsnub'					:	65																								,	#?	maximum allowed active snubber voltage during precharge of the precharge
                            'Ivec'					:	[0,0]																							,	#?	maximum allowed LV current during precharge of the precharge
                            'Vvce'					:	[0,0]																							,	#?	corresponding HV voltage vector for the max allowed current
							'ohmicOffset'			:	0																								,	#?	add ohmich offset to the reference voltage
							'enableOffset'			:	0																									#?	0->disable | 1->enable
						}
...
ACMCboost			= 	{																																	#*	boost mode controller
							...
							'LoLim'			  		:	PWM['Dmin']*PWM['M']/2   																		,	#?	lower saturation limit
							'CCM_Hi'	  			:	25																								,	#?	current threshold to switch to CCM mode
							'CCM_Lo'	  			:	5																								,	#?	current threshold to switch to DCM mode
...
LeftLeg_1			=	copy.deepcopy(Switches['AIMDQ75R040M2H'])																						#*	HV left leg switches parameters
LeftLeg_2			=	copy.deepcopy(Switches['AIMDQ75R040M2H'])																						#*	HV left leg switches parameters
RightLeg_1			=	copy.deepcopy(Switches['AIMDQ75R040M2H'])																						#*	HV right leg switches parameters
RightLeg_2			=	copy.deepcopy(Switches['AIMDQ75R040M2H'])																						#*	HV right leg switches parameters

#! Rectifier Switches
#?----------------------------------------------------------------------------------------------------------------------------------------------------------------------

Rectifier_1			=	copy.deepcopy(Switches['NVBYST0D6N08X'])																							#*	LV rectifier switches parameters
Rectifier_2			=	copy.deepcopy(Switches['NVBYST0D6N08X'])																							#*	LV rectifier switches parameters
Rectifier_3			=	copy.deepcopy(Switches['NVBYST0D6N08X'])																							#*	LV rectifier switches parameters
Rectifier_4			=	copy.deepcopy(Switches['NVBYST0D6N08X'])																							#*	LV rectifier switches parameters

#! Thermal Parameters
#?----------------------------------------------------------------------------------------------------------------------------------------------------------------------

Thermal 			=   {																																	#* 	thermal conditions
							'Tinit'             	:   62           		 																			,	#? 	initial temperature of semiconductors and heatsinks
							...
							'Twater'            	:	62                                          													,	#? 	coolant water temperature
                            'IMS_Rth'				:	0																								,	#?	IMS board thermal resistance with respect to hottest component
							'BuckBoost'				:	0																								,	#?	0->buck operation | 1->boost operation
                            'TempSelect_IMS'		:	0																								,	#?	0->no coupling | 1->IMS coupling
                            'R_IMS'					:	0																								,	#?	IMS board equivalent resistance
                            'Paux'					:	30																								,	#?	DCDC aux consumption
                            'HV_Modulation'			:	1																								,	#?	PWM modulation to extract switching values, 1->CCM | 2->DCM
                            'LV_Modulation'			:	1																								,	#?	PWM modulation to extract switching values, 1->buck | 2->boost
                            'Derating'	:	{																												#*	boundary coniditions for derating functions
								'Vmin'				:	6																								,	#?	min output voltage
                                'Vnom'				:	13.5																							,	#?	nom output voltage
                                'Vmax'				:	15.5																							,	#?	max output voltage
                                'Twater_nom'		:	62																								,	#?	nom water temperature
                                'Twater_max'		:	75																								,	#?	max water temperature
                                'Pmax_2s_nom'		:	2700																							,	#?	max 2-sec power at low water temp
                                'Pmax_2s_max'		:	1957																							,	#?	max 2-sec power at high water temp
                                'Pmax_10s_nom'		:	2160																							,	#?	max 10-sec power at low water temp
                                'Pmax_10s_max'		:	1822.5																							,	#?	max 10-sec power at high water temp
                                'Pcont_nom'			:	2025																							,	#?	max continuous power at low water temp
                                'Pcont_max'			:	1687.5																								#?	max continuous power at high water temp
							},
                            'PCB_IMS_SS':	{																												#*	temperature coupling on Basis DCDC IMS board and its components
                                ...
                                'Temps_Init'		:	[62,62,62,62,62]																				,	#?	components initial temperatures
...
SolverOpts			= 	{																																	#*	solver settings parameters for simulations
							'Solver'					: 	'auto'																						,	#?	solver to use for the simulation. Possible values are auto, dopri, radau, sdirk and esdirk for variable-step solvers and discrete for fixed-step solver
                            'JacobianComputation'		:	'analytical'																				,	#?	specifies how the solver calculates the Jacobian matrix required for the integration of stiff systems. Possible values are analytical and finite differences
							'StartTime'					:	0																							,	#?	start time specifies the initial value of the simulation time at the beginning of a simulation, in seconds
							'TimeSpan'					:	simParams['tSim']																			,	#?	simulation ends when the simulation time has advanced by the specified time span, in seconds
							'StopTime'					: 	simParams['tSim']																			,	#?	this parameter is obsolete. It is provided to keep old scripts working. Advise against using it in new code
							'Timeout'					:	0																							,	#?	maximum number of seconds that a simulation or analysis is allowed to run. After this period the simulation or analysis is stopped with a timeout error. A value of 0 disables the timeout.
							'MaxStep'					:	simParams['Maxstep']																		,	#?	maximum step size taken by the variable-step solvers
							'FixedStep'					:	simParams['Fixedstep']																		,	#?	simulation step size taken by the fixed-step solver
							'RelTol'					:	simParams['RelTol']																			, 	#?	specify the acceptable local integration errors for the individual state variables
                            'Refine'					:	simParams['Refine']																				#?	factor for generating additional output points in order to achieve smoother curves in scopes
						}
...
DCDC_Rail1 			= 	{
							...
							'AC_voltageSense'			:	AC_voltageSense							,
...
DCDC_Rail2 			= 	copy.deepcopy(DCDC_Rail1)
