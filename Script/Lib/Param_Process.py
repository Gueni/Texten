...
class ParamProcess:
    def __init__(self):
        ...
        self.postProcessing     =   PP.Processing()
        self.simutil            =   sutl.SimulationUtils()
    ...
    def MOSFETrdson(self, path, switch_name, type=""):
        ...
        *Args    :
        ...
        Rvec        = (dp.np.array(Rvec)).tolist()
        Xvec        = (dp.np.array(Xvec)).tolist()
    ...
    def DiodeVI_data(self, DiodeVIPath, diode_name):
        ...
        *Args    :
        ...
        Vvec    =   (dp.np.array(V_I[1:])).T      # Voltage array, transposed
        Ivec    =   dp.np.array(V_I[0])           # Current array
    ...
    def getCmos(self, capacitance, Cmos_tr, cmos_er, blockingVoltage, time_energy):
            ...
            *Args    :
            ...
            # Get the index of the closest voltage to the blockingVoltage
            Cmos_idx        = self.simutil.postProcessing.get_index(capacitance, blockingVoltage, 0)
    ...
    def Mags_FreqRes_Data(self, FreqResistancePath, mag_name):
        ...
        *Args    :
        ...
        for i in range(len(mag_name)):
            ...
            # Convert units
            R           = (dp.np.array(R_F[1:])*1e-3).tolist()  # mOhms -> Ohm
            F           = (dp.np.array(R_F[0])*1e3).tolist()    # kHz -> Hz
        ...
        *Args   :
        ...
        def build_freq_Z_from_lists(cols):
            ...
            *Args   :
            ...
            freq                            = dp.np.asarray(freq_list, dtype=float)
            dB                              = dp.np.asarray(dB_list,   dtype=float)
            deg                             = dp.np.asarray(deg_list,  dtype=float)

            # Convert dB/deg -> complex Z
            mag                             = 10.0 ** (dB / 20.0)
            Z                               = mag * dp.np.exp(1j * dp.np.deg2rad(deg))
            return freq, Z
        ...
        # Extract data
        freqs, dBs, degs        = self.postProcessing.extractArrays(full_path)
        freq, Z                 = build_freq_Z_from_lists([freqs, dBs, degs])

        # Selection
        freq_sel, Z_sel         = po.select_and_decimate(freq, Z, FMIN, FMAX, N_POINTS)
        s                       = 1j*2*dp.np.pi*freq_sel

        problem                 = po.ImpedanceProblem(s, Z_sel, PARAM_SPACE)
        algorithm               = dp.GA(pop_size=POP_SIZE, eliminate_duplicates=True)
        termination             = dp.get_termination("n_gen", N_GEN)

        result_ga               = dp.optmin(problem, algorithm, termination, verbose=VERBOSE)
        ...
        if DO_NM:
            ...
            p0                  = dp.np.maximum(params_ga, po.POS_FLOOR)
