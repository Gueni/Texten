...
class MapStrategy(ABC):
    """
    Strategy interface: each sweep mode (Normal, Permute, WCA, Sensitivity, MonteCarlo) implements
    generate(config) to turn a sweep configuration into a parameter map. Never instantiated directly.
    """
    @abstractmethod
    def generate(self, config):
        """
        Build the parameter sweep map for this strategy from the given config.

        *Args:
            config (dict) : Sweep configuration -- always includes "sweepMatrix" (the X1-X10 parameter
                            lists), plus whatever else this strategy needs (perturbation, points, Tols, seed, ...).
        """
        pass

class Map():
    """
    Base class shared by every concrete sweep strategy: holds the sweep state (sweepMatrix, Map,
    Iterations) and the sequential/Cartesian map-generation helpers every strategy is built from.
    """
    def __init__(self):
        self.sweepMatrix    = []
        self.Map            = ''
    ...
    def GenerateNormalMap(self, matrix):
        """
        Generate a sequential (non-Cartesian) sweep: pads every X1-X10 list to the length of the
        longest one by repeating its last value, then transposes so each row is one iteration.

        *Args   :
            matrix  (list)          : Parameter matrix (X1-X10 lists)

        !Returns:
            tuple                   : (parameter combinations, number of combinations)
    ...
    def GeneratePermuteMap(self, matrix):
        """
        Generate every combination of the X1-X10 parameter values (a full Cartesian product).
    ...
    def returnResults(self):
        """
        Package this map's results for the caller, after generate() has populated them.

        !Returns:
            tuple   :   (sweepMatrix, Map, Iterations) -- the raw X1-X10 lists used, the generated
                        parameter combinations, and how many iterations/rows the map contains.
        """
        return self.sweepMatrix, self.Map, self.Iterations

class NormalMap(MapStrategy, Map):
    """
    Sequential sweep strategy: each X1-X10 list is stepped through in lockstep (padded to the
    longest list's length), rather than combined into every possible permutation.
    """
    def generate(self, config):
        """
        *Args   :
            config (dict)   : Must contain "sweepMatrix" (the X1-X10 parameter lists).

        !Returns:
            tuple            : (sweepMatrix, Map, Iterations), from returnResults().
        """
...
class PermuteMap(MapStrategy, Map):
    """
    Full Cartesian product sweep strategy: every combination of the X1-X10 parameter values is
    generated, rather than stepping through them in lockstep.
    """
    def generate(self, config):
        """
        *Args   :
...
class WcaMap(MapStrategy, Map):
    """
    Worst-Case Analysis (WCA) sweep strategy: for each active parameter (given as [nominal,
    tolerance] or [nominal, tolerance, min, max]), sweeps every combination of its upper/lower
    tolerance bound (2**n_active iterations), plus one final row at all nominal values.
    """
    def generate(self, config):
        """
        *Args   :
            config (dict)     : Must contain "sweepMatrix" (the X1-X10 parameter lists, WCA-format entries).
...
class SensitivityMap(MapStrategy, Map):
    """
    Sensitivity sweep strategy: perturbs each active parameter by perturbation_value (one at a
    time, others held at nominal), then runs a normal (sequential) sweep over the result.
    """
    def generate(self, config):
        """
        *Args   :
            config (dict) : Must contain "sweepMatrix" (the X1-X10 parameter lists), "perturbation"
                            (the perturbation size), and "nvars" (number of active parameters, only
                            required when every X-list is zero -- see apply_perturbation()).
...
class MonteCarloMap(MapStrategy, Map):
    """
    Monte Carlo sweep strategy: draws random values for each active parameter, within +/- its
    tolerance, then runs a normal (sequential) sweep over the generated points.
    """
    def generate(self, config):
        """
        *Args   :
            config (dict)     : Must contain "sweepMatrix" (the X1-X10 parameter lists), "points"
                                (random points per variable), "Tols" (tolerances), and "seed"
                                (RNG seed, for reproducibility -- see apply_rand_tol()).
...
class MapContext:
    """
    Picks and runs one of the sweep strategies (Normal, Permute, WCA, Sensitivity, MonteCarlo)
    by name, without the caller needing to know which concrete class implements it.

    *Args:
        map_strategy (str)   : Name of the strategy to select initially -- one of "NORMAL", "PERMUTE",
                               "WCA", "MONTECARLO", "SENSITIVITY".
    """
    ...
    def set_map_strategy(self, map_strategy):
        """
        Switch to a different sweep strategy.

        *Args:
            map_strategy (str)   : Name of the strategy to switch to -- one of the keys in self.maps.
        """
        self.map_strategy = self.maps[map_strategy]

    def generate_map(self, config):
        """
        Run the currently selected strategy's generate() with the given config.

        *Args   :
            config (dict)   : Sweep configuration for whichever strategy is currently selected.

        !Returns:
            tuple            : (sweepMatrix, Map, Iterations), from the strategy's returnResults().
        """
