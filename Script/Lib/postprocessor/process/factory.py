...
class ProcessorFactory():
    """
    Turns a ProcessorConfig's process_dict into actual instantiated IProcess objects, looking
    each one up in ProcessRegistry by its configured processor type.
    """
    def build_process_list(self, config: ProcessorConfig) -> list[IProcess]:
        """
        Build one concrete IProcess instance per entry in config.process_dict, each configured
        with its own result name and data slice.

        *Args   :
            config (ProcessorConfig)  : Config supplying process_dict (which processes to build), process_cfg
                                        (per-process-type config dataclasses), and slice_map (named data slices).

        !Returns:
            list[IProcess]             : The built, ready-to-run process instances, in process_dict's iteration order.
        """
