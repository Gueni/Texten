...
class RT_Box:
    ...
    *Args:
        model_name      : (str)             :   Used to build the default codegen executable path: '<model_name>_codegen/<model_name>.elf'.
        host_name       : (str)             :   Hostname or IP address of the RT Box (e.g. "rtbox-123.local").
        method          : (str , optional)  :   RPC transport to use, either "XML" (XML-RPC) or "JSON" (JSON-RPC). Defaults to "XML".
        port            : (int , optional)  :   TCP port the RT Box listens on for RPC connections. Defaults to '9998'.
    ...
    def __init__(
        ...
        self.model_name         = model_name   #todo either given or derived from the codegen path
        self.host_name          = host_name
        self.method             = method
        self.port               = port
        self.enabled            = enabled

        # Skip DNS resolution entirely when disabled, so this class can be run with no link to the RT Box at all.
        self.ip                 = ""
        self.host_address       = ""
    ...
    def from_config(cls, config_path: str | Path) -> "RT_Box":
        """
        Toggle use on/off without touching code: set "RT": false in the json file to make every RT Box call a no-op 
        (e.g. for running other simulations with plecs),and "RT": true when running against the actual RT Box.

        *Args:
            RT              (bool)          : Master enable switch. Missing/false disables the instance.
            model_name      (str)
            host_name       (str)
            method          (str, optional) : "XML" or "JSON". Defaults to "XML"
            port            (int, optional) : Defaults to DEFAULT_PORT
            codegen_path    (str, optional)
            config_path     (str)           : Path to the JSON config file.

        !Returns:
                                            : Configuration for RT Box
        """
        config_path                 =   Path(config_path)
        with open(config_path) as f :   config = json.load(f)

        # config["RT_BOX"] when reading the project's Input_vars.json; falls back
        # to the whole dict when reading a standalone RT-only config file.
        rt_config                   =   config.get("RT_BOX", config)

        return cls(
                    enabled         =   bool(rt_config.get("RT", False)     ),
                    model_name      =   rt_config.get("model_name",     ""  ),
                    host_name       =   rt_config.get("host_name",      ""  ),
                    method          =   rt_config.get("method")   or "XML"   ,
                    port            =   rt_config.get("port")     or cls.DEFAULT_PORT,
                    codegen_path    =   rt_config.get("codegen_path")  or "",
                )
    ...
    def run(
        ...
        *Args:
        ...
        ?Example:
            ...
            data    = rt.run(set_values={"Input": [5.0]},capture_blocks=["Capture1", "Capture2"])
        """
        if not self.enabled:
