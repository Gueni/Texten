...
class RTBoxError(Exception):
    ...
    ?NOTE:
        ...
        every failure it wraps (a connection error from rt_connect, a file error from rt_load,
...
class RT_Box:
    ...
    ?NOTE:
        url/port are consumed the exact same shape PlecsRPC's own url/port are (concatenated directly,
        "url:port", no re-resolution) -- because in production (RunScripts.py) they ARE dp.url/dp.port,
        the same values PlecsRPC itself gets. That's a genuinely different endpoint than the RT Box
        (PLECS runs locally, e.g. "http://127.0.0.1"; the RT Box is separate hardware elsewhere on the
        network), so dp.url/dp.port need to actually point at the RT Box before this connects for real.
        Unlike PlecsRPC, the RT Box's own RPC server requires the URL to end in "/RPC2" -- rt_connect()
        appends that itself, so url shouldn't include it.

    *Args:
        url             : (str)             :   Full address including scheme, e.g. "http://rtbox-123.local" -- same shape as
                                                PlecsRPC's own url param, just pointed at different hardware.
        ...
        codegen_path    : (str , optional)  :   Explicit path to the compiled executable. If omitted, it is derived from 'model_name'.
        ...
        codegen_only    : (bool, optional)  :   When 'True', run() stops right after generating code -- it never connects to, uploads to, or
                                                starts anything on actual RT Box hardware. Useful for building an .elf without hardware attached,
                                                or batch-generating several before deploying any of them. Has no effect when enabled=False (run()
                                                is already a full no-op then). Defaults to 'False' (the full connect/load/start/.../stop routine).
    """
    ...
    def __init__(
        ...
        self.codegen_only       = codegen_only
        ...
        self.codegen             = Path(codegen_path) if codegen_path   else Path(f"{self.model_name}_codegen") / f"{self.model_name}.elf"
        self.server              = None

    @classmethod
    def from_config(cls, config_path: str | Path) -> "RT_Box":
        """
        Build an RT_Box, reading only the "RT" flag from the given JSON config file for its
        enabled state -- url/port/model_name are NOT read from JSON at all: in production
        (RunScripts.py) they're sourced exactly like PlecsRPC gets its own (dp.url, dp.port,
        self.JS['modelname']), not from config. Mainly useful for quick/standalone testing
        against a minimal JSON file containing just {"RT": true/false}.

        *Args:
            config_path (str | Path)  :   Path to a JSON file with an "RT" key (missing/false -> disabled).

        !Returns:
            RT_Box  :   A new instance with .enabled set from config_path's "RT" key; url/port/model_name left at their defaults.
        """
        config_path                 =   Path(config_path)
        with open(config_path) as f :   config = json.load(f)

        return cls(enabled=bool(config.get("RT", False)))

    #?--------------------------------------------------------------------
    #? Code generation (PLECS-side, before any RT Box hardware is touched)
    #?--------------------------------------------------------------------

    def generate_code(self, rpc, outputDir: str | None = None) -> Path:
        """
        Triggers PLECS Coder to generate real-time code for the model rpc has open (rpc.path, set by
        rpc.Open_Model()), over RPC, using rpc's own OptStruct (ModelVars/SolverOpts/AnalysisOpts, built
        by rpc.optStruct() from dp.mdlVars etc.) -- the exact same values already used for a normal
        simulation run via rpc.LaunchSim(). This is the RPC equivalent of clicking the Coder Options
        window's Build button, and it's a PLECS-software operation (via rpc's own connection), not an
        RT Box one -- nothing here talks to the RT Box itself.

        ?NOTE:
            Assumes rpc.OptStruct is the single-dict shape ({'ModelVars': ..., ...}, not a list of dicts) --
            i.e. rpc.optStruct() was called for one instance, non-parallel. Code generation targets a
            single build; it isn't meaningful to generate multiple parallel .elf variants from one call.

        *Args:
            rpc         (PlecsRPC)       : An already-connected PlecsRPC instance with Open_Model() and
                                           optStruct() already called (rpc.path and rpc.OptStruct set).
            outputDir   (str, optional)  : Where to place the generated files. Defaults to the same directory
                                           as the model file (rpc.path's folder), so the result lands as
                                           "<modelname>.elf" right next to the .plecs file.

        !Returns:
            Path    :   Path to the generated "<modelname>.elf", for the caller to use directly (e.g. to
                       set self.codegen before rt_load(), or to copy it into a results folder) rather than
                       needing to re-derive it.
        """
        modelname   = os.path.splitext(os.path.basename(rpc.path))[0]
        if outputDir is None :   outputDir = os.path.dirname(rpc.path)

        rpc.server.plecs.codegen(modelname, rpc.OptStruct, outputDir)

        return Path(outputDir) / f"{modelname}.elf"
    ...
    def rt_load(self) -> None:
        ...
        !Raises:
            RTBoxError If the executable file is missing/unreadable, or the RT Box rejects the upload.
        """
    ...
    def run(
        ...
        """
        Run the full RT Box workflow end to end: optionally generate code first, then connect, load,
        start, optionally write Programmable Values, optionally read Data Capture blocks, then stop.

        *Args:
            rpc                     (PlecsRPC, optional)            :   An already-connected PlecsRPC instance
                                                                        (Open_Model() and optStruct() already
                                                                        called). When given, generate_code(rpc)
                                                                        runs first and its result becomes
                                                                        self.codegen for this run, so the
                                                                        freshly-built .elf is what gets uploaded --
                                                                        not whatever codegen_path/derived guess
                                                                        was set at construction. Omit to run()
                                                                        using self.codegen as it already stands.
        ...
        !Returns:
                                    (dict)                          :   Captured data keyed by capture block name. Empty if capture_blocks was not given,
                                                                        if the instance is disabled, or if self.codegen_only is True (nothing was captured
                                                                        since the RT Box hardware routine never ran).

        ?Example:
            rt      = RT_Box(url=dp.url, port=dp.port, model_name="psfbConverterFBR", enabled=dp.JSON["RT"])
            data    = rt.run(rpc=plecs_rpc_obj, capture_blocks=["Capture1", "Capture2"])

            rt_prep = RT_Box(url=dp.url, port=dp.port, model_name="psfbConverterFBR", enabled=True, codegen_only=True)
            rt_prep.run(rpc=plecs_rpc_obj)   # builds the .elf only -- no hardware touched at all
        """
        if not self.enabled:
        ...
        if rpc is not None:
            self.codegen = self.generate_code(rpc)

        if self.codegen_only:
            print("RT Box codegen_only=True — code generated, skipping connect/load/start/stop.")
            return {}
